...
class PriorityQueue:
    ...
    def check_ri(self, i=0):
        left = (2*i)+1
        right = (2*i)+2
        if left < len(self.array) and self.array[left]<self.array[i]:
            raise ValueError('RI violated: ' + str(self.array[left]) + ' ' + str(self.array[i]))
        if right < len(self.array) and self.array[right]<self.array[i]:
            raise ValueError('RI violated: ' + str(self.array[right]) + ' ' + str(self.array[i]))
    ...
    def append(self, key):
        ...
        node = len(self.array)-1
        while 0 < node and self.array[node] < self.array[(node-1)//2]:
            self.array[node], self.array[(node-1)//2] = self.array[(node-1)//2], self.array[node]
            node = (node-1)//2
    ...
    def pop(self):
        ...
        if len(self.array) == 0:
            raise ValueError('Que is empty')
        self.array[len(self.array)-1], self.array[0] = self.array[0], self.array[len(self.array)-1]
