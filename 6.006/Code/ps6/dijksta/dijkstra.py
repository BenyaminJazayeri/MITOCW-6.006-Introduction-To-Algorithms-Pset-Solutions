...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
        parent = {source:None}
        keys = {source:NodeDistancePair(source,0)}
        q = PriorityQueue()
        q.insert(keys[source])
        count = 0
        while q:
            nodedistpair = q.extract_min()
            node = nodedistpair.node
            node_dist = nodedistpair.distance
            count += 1
            if node == destination:
                result = []
                while node != None:
            ...
            for neighbor in node.adj:
                if neighbor not in keys:
                    neighbor_dist = node_dist + weight(node,neighbor)
                    keys[neighbor] = NodeDistancePair(neighbor,neighbor_dist)
                    q.insert(keys[neighbor])
                    parent[neighbor] = node
                elif keys[neighbor].distance > node_dist + weight(node,neighbor):
                    keys[neighbor].distance = node_dist + weight(node,neighbor)
                    q.decrease_key(keys[neighbor])
                    parent[neighbor] = node
